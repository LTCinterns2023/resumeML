...
function searchKeywords(keywords) {  // search using keywords
...
function searchwithLabel(lbl, keywords) {  // search using keywords

    //with labels 
    return j.filter(el => { 
        return el.annotation.some(({label, points}) => label == lbl && points[0].text == keywords)
    })
